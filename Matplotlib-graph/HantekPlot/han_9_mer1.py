...
####################################
    ...
    t2 = len(timing_data2)
    A2_max = max(voltage_data2)
    Amp2  = str(A2_max)
    A2_min = min(voltage_data2)
    freq2 = 1
    for i in range(t2):
        if(voltage_data2[i]==A2_max):
            t22 = round((timing_data2[i]),4)
            #print("t_max:",t11)
            if(t22!=0):
                f22 = 2/(t22*1)
                freq2=f22+70
        if(voltage_data2[i]==A2_min):
            t2_min = round((timing_data2[i]),4)
